...
export default function HomePage() {
  ...
  const handleFilter = async () => {
    ...
    try {
      const response = await axios.get("/api/filter/itineraryFilter", {
        params,
      });
    ...
    if (tagId === "all") {
      ...
      try {
        const response = await fetch(
          `/api/filter/filterByPreferenceItineraries/${tagId}`
        );
  ...
  const handlePrefAChange = async (event) => {
    ...
    if (tagId === "all") {
      ...
      try {
        const response = await fetch(
          `/api/filter/filterByPreferenceActivities/${tagId}`
        );
  ...
  const handleFilterActivity = async () => {
    ...
    try {
      const response = await axios.get("/api/filter/filterActivities", {
        params,
      });
      if (response.data == null) {
        setActivities([]);
      ...
      <main className="flex-grow">
        ...
        <section className="container mx-auto px-4 py-12">
          ...
          <div>
            ...
            <button className="border rounded p-2 mr-2" onClick={handleFilter}>
              Filter Itineraries
            </button>
            <button
              className="border rounded p-2"
              onClick={resetFiltersAndFetchAll}
            >
              All
            </button>
          </div>
        ...
        <section className="container mx-auto px-4 py-12">
          ...
          <div>
            ...
            <button
              className="border rounded p-2"
              onClick={resetFiltersAndFetchAllActivity}
            >
              All
            </button>
