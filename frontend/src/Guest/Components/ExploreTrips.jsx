...
const ExploreTrips = () => {
  ...
  }, [itineraries, tripSearch, tripSort, startDate, endDate, minPrice, maxPrice, languageOfTour, selectedPrefItinerary]);
  ...
  const filterAndSortItineraries = () => {
    ...
    if (tripSearch) {
      filtered = filtered.filter(trip => 
        trip.name && trip.name.toLowerCase().includes(tripSearch.toLowerCase())
      );
    }

    if (startDate) {
      filtered = filtered.filter(trip => 
        trip.startDate && new Date(trip.startDate) >= startDate
      );
    }

    if (endDate) {
      filtered = filtered.filter(trip => 
        trip.endDate && new Date(trip.endDate) <= endDate
      );
    }

    if (minPrice) {
      filtered = filtered.filter(trip => 
        trip.priceOfTour && parseFloat(trip.priceOfTour) >= parseFloat(minPrice)
      );
    }

    if (maxPrice) {
      filtered = filtered.filter(trip => 
        trip.priceOfTour && parseFloat(trip.priceOfTour) <= parseFloat(maxPrice)
      );
    }

    if (languageOfTour) {
      filtered = filtered.filter(trip => 
        trip.languageOfTour && trip.languageOfTour.toLowerCase().includes(languageOfTour.toLowerCase())
      );
    }

    if (selectedPrefItinerary && selectedPrefItinerary !== 'all') {
      filtered = filtered.filter(trip => 
        trip.PreferenceTag && trip.PreferenceTag.includes(selectedPrefItinerary)
      );
    }
  ...
  return (
    <section className="container mx-auto px-4 py-12 bg-gray-50">
      <h2 className="text-3xl font-bold mb-8 text-left text-gray-800">Explore Trips</h2>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Language of Tour</label>
            <div className="relative">
              <Globe className="absolute text-gray-400 top-3 left-3" size={16} />
              <input
                type="text"
          ...
          <div>
            ...
            <div className="relative">
              <SortAsc className="absolute text-gray-400 top-3 left-3" size={16} />
              <select
                className="w-full pl-10 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={tripSort}
      ...
      {isLoading && <p className="text-center text-gray-600">Loading trips...</p>}
            ...
            {filteredItineraries.map((trip) => (
              ...
              >
                ...
                <div className="p-4 h-48 overflow-y-auto">
                  <h3 className="font-bold text-lg text-gray-800 mb-2 line-clamp-2">{trip.name}</h3>
                  <p className="text-sm text-gray-600 mb-2 line-clamp-2">{trip.location}</p>
                  <p className="text-lg font-semibold text-blue-600 mb-2">${trip.priceOfTour}</p>
